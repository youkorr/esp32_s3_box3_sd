...
void SDFileServer::handle_download(AsyncWebServerRequest *request, std::string const &path) const {
  ...
  auto *response = request->beginChunkedResponse("application/octet-stream", [this, path](uint8_t *buffer, size_t maxLen, size_t index) -> size_t {
    // Read chunk from file
    size_t bytes_read = this->sd_mmc_card_->read_file_chunk(path, buffer, maxLen, index);
    
    // If we've read 0 bytes, we're done
    if (bytes_read == 0) {
      return 0;
    }
  ...
  response->addHeader("Content-Length", String(file_size).c_str());
